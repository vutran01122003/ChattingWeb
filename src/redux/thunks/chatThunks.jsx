...
export const getConversation = createAsyncThunk(
    ...
    async (conversationId, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue(err.response?.data?.message || "Failed to create/get conversation");
        }
    }
);
...
export const getConversationMessages = createAsyncThunk(
    ...
    async ({ conversationId, page = 1, limit = 20 }, { rejectWithValue }) => {
        try {
            const clientId = localStorage.getItem("clientId");
            const response = await axios.get(`/conversations/${conversationId}/messages?page=${page}&limit=${limit}`, {
                headers: { "x-client-id": clientId }
            });
            return response.data.metadata;
        } catch (err) {
            return rejectWithValue(err.response?.data?.message || "Failed to get conversation details");
        }
    }
);
...
export const revokeMessage = createAsyncThunk("message/revokeMessage", async ({ messageId }, { rejectWithValue }) => {
    try {
        const clientId = localStorage.getItem("clientId");
        ...
        return rejectWithValue(err.response?.data?.message || "Failed to mark conversation as read");
    }
});
