...
function SideBar({ auth, setSelectedUser, selectedUserId }) {
    ...
    const renderUserAvatar = (user) => {
        ...
        } else if (user.initials === 'TD' || user.initials === 'NT') {
            return (
                <div className="w-10 h-10 rounded-full bg-pink-400 flex items-center justify-center text-white mr-3">
                    {user.initials}
                </div>
            );
        } else {
                            ...
                            <div className="flex-1 overflow-y-auto">
                                {searchList.map((user, index) => (
                                    <div key={user._id}>
                                        ...
                                        {index < searchList.length - 1 && <hr className="border-gray-100" />}
                                    </div>
                                ))}
                            </div>
                        )}
