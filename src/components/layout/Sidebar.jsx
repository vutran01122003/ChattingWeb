...
function SideBar({ setSelectedUser, selectedUserId }) { 
    ...
    <div className="w-[300px] flex flex-col h-screen bg-white">
      ...
      <div className="flex items-center px-3 text-sm">
        ...
        <button
          ...
          className={`py-2 mr-4 ${activeTab === "unread" ? "text-blue-600 font-semibold border-b-2 border-blue-600" : "text-gray-600"}`}
        >
          Chưa đọc
        </button>
      </div>
        ...
        {filteredUsers?.map((user) => (
          ...
          >
            ...
            <div className="text-xs text-gray-400">
              {user.timestamp ? `${Math.floor((new Date() - new Date(user.timestamp)) / 60000)} phút trước` : 'No time'}
            </div>
          </div>
        ))}
      </div>
