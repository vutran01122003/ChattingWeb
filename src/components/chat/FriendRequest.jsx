...
function FriendRequest({
  ...
  const loadFriendshipStatus = () => {
    ...
    dispatch(checkBlockedUser({ userId: selectedUser[0]._id }));
    dispatch(checkIsBlockedUser({ userId: selectedUser[0]._id }));
    ...
    if (selectedUser && conversation) {
      ...
      if (conversationType !== "group") {
        ...
        const handleSocketRefresh = (data) => {
          ...
          if (
            data.fromUserId === selectedUser[0]._id ||
            data.toUserId === selectedUser[0]._id
          ) {
    ...
    if (selectedUser) {
      ...
      if (socket?.connected) {
        if (!selectedUser[0].conversationId) {
          console.warn(
            `conversationId không hợp lệ cho selectedUser: ${selectedUser._id}`
          );
        } else {
