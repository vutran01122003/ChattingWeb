    ...
    if (isRevoked) {
        ...
        if (msg.attachments?.length > 1) {
            return (
                <>
                    <div
                        className={`mb-6 flex ${isMe ? "justify-end" : "justify-start"}`}
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-2 max-w-lg rounded-lg p-2 relative">
                            ...
                            {hoveredMessageId === msg._id && (
                                <div
                                    className={`message-actions absolute top-1/2 transform -translate-y-1/2 z-10
        ...
        return (
            <>
                <div
                    className={`mb-6 flex ${isMe ? "justify-end" : "justify-start"} relative group`}
                    onMouseEnter={(e) => handleMouseEnter(msg, e)}
                    onMouseLeave={() => handleMouseLeave()}
                    ref={(el) => (messageRefs.current[msg._id] = el)}
                >
                        ...
                        {msg.attachments?.length > 0 && (
                            <div className={`${msg.attachments.length > 1 ? "grid grid-cols-2 gap-2" : ""} mb-2`}>
                                {msg.attachments.map((file) => (
                                    <FileAttachment key={file._id} file={file} onImageClick={onImageClick} />
                                ))}
                            </div>
                        )}
