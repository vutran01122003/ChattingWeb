                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                {readUsers.slice(0, 5).map(user => (
                                    <div key={user._id} className="flex items-center mb-2">
                                        ...
                                        <div>{user.full_name.length > 20 ? user.full_name.substring(0, 10) + '...' : user.full_name}</div>
                                    </div>
                                ))}
                                {readUsers.length > 5 && (
            ...
            <div className="bg-white rounded-lg w-full max-w-md">
                ...
                <div className="p-4">
                    ...
                    <div className="flex items-center">
                        {!isMe ? (
                            <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden mr-3">
                                <img src={otherUser[0].avatar_url} alt="User avatar" className="w-full h-full object-cover" />
                            </div>
                        ) : <></>}
                        <div className="bg-blue-100 p-3 rounded-lg mb-4">
                            <div className="text-gray-700">{message.sender.full_name}</div>
                            <div>{message.content}</div>
                            <div className="text-right text-xs text-gray-500 mt-1">{formatDateHeader(new Date(message.createdAt || Date.now()))}</div>
                        </div>
                    </div>
...
export default function MessageMenu({ isMe, currentYScroll, onClose, otherUser, msg, handleDeleteMessage, handleRevokeMessage }) {
